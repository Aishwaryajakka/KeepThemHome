...
import { ArrowLeft, ArrowRight, ExternalLink, LockKeyhole, RotateCcw } from 'lucide-react';
import { Link } from 'react-router-dom';
import { Button } from '@/components/ui/button';
import { StatusBadge } from '@/components/ui/status-badge';
import type { BarrierType, HousingGoal, HousingSituation, HousingTiming, PetType, TriStateAnswer } from '@/types/assessment';
import { matchResources } from '@/data/resources';
import {
...
const EMPTY_DOMAIN = { primarySupportPossible: '', bridgeAvailable: '', alternativeAvailable: '', urgency: '' } as const;
const factLabel = (field: string, hypothetical = false) => {
  const labels: Record<string, string> = {
    ...
    primarySupportPossible: hypothetical ? 'Primary support confirmed' : 'Primary support is not confirmed',
    bridgeAvailable: hypothetical ? 'Bridge support confirmed' : 'Bridge support is not confirmed',
    alternativeAvailable: hypothetical ? 'Alternative support confirmed' : 'Alternative support is not confirmed',
    costReductionPossible: hypothetical ? 'Cost pressure can be reduced' : 'Cost reduction is not confirmed',
    careAccessPossible: hypothetical ? 'Care access confirmed' : 'Care access is not confirmed',
    careSupportAvailable: hypothetical ? 'Care support confirmed' : 'Care support is not confirmed',
    householdAdaptationPossible: hypothetical ? 'Household adaptation confirmed' : 'Household adaptation is not confirmed',
  };
  return labels[field] ?? 'Constraint to confirm';
};
...
export const HousingActionPlan: React.FC<HousingActionPlanProps> = ({
  ...
  const effectiveTiming = primaryBarrier === 'housing' ? timing : domainAnswers.urgency;
  const fallbackResources = matchResources(primaryBarrier, situation, effectiveTiming, goal);
  const backendResources = backendPlan?.interventions.flatMap(({ resources }) => resources).slice(0, 3) ?? [];
  const resources = backendResources.length ? backendResources : fallbackResources;
  const triState = (value: TriStateAnswer) => value === 'yes' ? true : value === 'no' ? false : 'unknown' as const;
  const localFacts = useMemo<NormalizedHousingCase>(() => ({
    primaryBarrier, contributingBarriers, situation, urgency: effectiveTiming, goal, costConstraint: costConstraint || null,
    constraints: {
      ...
      primarySupportPossible: triState(domainAnswers.primarySupportPossible), bridgeAvailable: triState(domainAnswers.bridgeAvailable),
      alternativeAvailable: triState(domainAnswers.alternativeAvailable), safetyManageable: primaryBarrier === 'behavior' ? triState(domainAnswers.primarySupportPossible) : 'unknown',
      costReductionPossible: 'unknown', careAccessPossible: 'unknown', careSupportAvailable: 'unknown', householdAdaptationPossible: 'unknown',
    },
  }), [contributingBarriers, costConstraint, domainAnswers, effectiveTiming, goal, primaryBarrier, situation]);
  ...
  useEffect(() => {
    ...
    let active = true;
    void caseApi.getRetentionPaths(backendCaseId, []).then(({ paths: results }) => { if (active) setPaths(results.length ? results : localPaths(localFacts)); }).catch(() => { if (active) setPaths(localPaths(localFacts)); });
    return () => { active = false; };
  ...
  useEffect(() => {
    ...
    for (const path of paths) {
      ...
      const request = backendCaseId ? caseApi.getPathEvidence(backendCaseId, path.key) : caseApi.previewPathEvidence({
        pathKey: path.key, primaryBarrier, situation: situation || null, urgency: effectiveTiming || null, goal: goal || null,
    ...
    return () => { active = false; };
  }, [appliedChanges, backendCaseId, contributingBarriers, costConstraint, effectiveTiming, goal, paths, primaryBarrier, situation]);
  ...
  return <div className="mx-auto w-full max-w-[1360px] px-4 py-8 sm:px-6 sm:py-12 lg:px-10 lg:py-16">
    ...
    <section className="mb-10 grid gap-6 rounded-2xl border border-[var(--sage)]/40 bg-white/75 p-5 sm:grid-cols-[180px_1fr] sm:p-7" aria-label={`${displayName} case context`}>
      ...
      <div className="grid gap-5 sm:grid-cols-2"><div><p className="text-sm font-bold text-[var(--forest)]">From your story</p><div className="mt-3 flex flex-wrap gap-2"><span className="rounded-md bg-[var(--forest)] px-3 py-2 font-semibold text-white">{factorLabel(primaryBarrier)}</span>{contributingBarriers.map((barrier) => <span key={barrier} className="rounded-md bg-[var(--warm-sand)]/45 px-3 py-2 font-semibold">{factorLabel(barrier)}</span>)}{effectiveTiming && <span className="rounded-md bg-[var(--surface-soft)] px-3 py-2 font-semibold">{effectiveTiming === 'This week' ? '7 days' : effectiveTiming}</span>}</div></div><div><p className="text-sm font-bold text-[var(--forest)]">Confirmed by you</p><p className="mt-3 text-base font-medium">{primaryBarrier === 'housing' ? (goal === 'Stay where I am' ? 'Stay in current home' : goal === 'Move' ? 'Open to moving' : 'Open to staying or moving') : `${factorLabel(primaryBarrier)} is the primary pressure`}</p></div></div>
    </section>
    ...
    {paths ? <section aria-labelledby="paths-heading"><h2 id="paths-heading" className="sr-only">Compared paths</h2><div className="grid gap-5 lg:grid-cols-3">
      {paths.map((path) => { const selected = selectedKey === path.key; const count = path.blockers.length; return <article key={path.key} className={`motion-safe:transition-all motion-safe:duration-300 relative flex min-h-[360px] flex-col rounded-2xl border-2 p-6 ${selected ? 'border-[var(--forest)] bg-[var(--sage)]/10 shadow-[0_20px_55px_rgba(46,84,64,.14)]' : 'border-[var(--border-warm)] bg-white'}`}>
        {selected && <span className="absolute right-5 top-5 text-xs font-bold text-[var(--forest)]">Selected</span>}
        <StatusBadge status={path.status} className="self-start px-3 py-2 text-base" /><h3 className="mt-6 font-serif text-3xl leading-tight text-[var(--forest)]">{path.title.replace('your pet', displayName)}</h3><p className="mt-5 text-base font-semibold text-[var(--text-muted)]">{count ? `${count} ${count === 1 ? 'thing still needs' : 'things still need'} to be resolved` : 'No blockers identified'}</p><Button type="button" aria-pressed={selected} onClick={() => selectPath(path)} variant={selected ? 'default' : 'outline'} className={`brand-focus mt-auto min-h-12 w-full rounded-full ${selected ? 'bg-[var(--forest)] text-white' : 'border-[var(--forest)] text-[var(--forest)]'}`}>{selected ? 'Selected path' : 'Explore this path'}</Button>
      </article>; })}
    </div></section> : null}
    ...
    {onSavePlan && <section className="mt-12 rounded-2xl bg-[var(--forest)] p-6 text-[var(--cream)] sm:p-8" aria-live="polite">{!signedIn ? <><h2 className="font-serif text-3xl">Want to come back to {displayName}’s plan?</h2><p className="mt-2 text-sm text-[var(--cream)]/80 sm:text-base">Sign in to save this plan and continue where you left off.</p></> : saveStatus === 'saved' ? <h2 className="font-serif text-3xl">{displayName}’s plan is saved.</h2> : <><h2 className="font-serif text-3xl">Save {displayName}’s plan</h2><p className="mt-2 text-sm text-[var(--cream)]/80 sm:text-base">Keep this plan in My Pets so you can return to it later.</p></>}<div className="mt-5 flex flex-col items-start gap-3 sm:flex-row sm:items-center">{saveStatus !== 'saved' && <Button type="button" onClick={onSavePlan} disabled={!authLoaded || saveStatus === 'saving'} className="brand-focus w-full bg-[var(--cream)] text-[var(--forest)] hover:bg-[var(--warm-sand)] sm:w-auto">{saveStatus === 'saving' ? `Saving ${displayName}’s plan…` : saveStatus === 'error' ? 'Try again' : signedIn ? `Save ${displayName}’s plan` : `Sign in to save ${displayName}’s plan`}</Button>}{saveStatus === 'saved' && <Link to="/my-pets" className="brand-focus rounded font-bold underline underline-offset-4">View My Pets <ArrowRight className="ml-1 inline h-4 w-4" aria-hidden="true" /></Link>}{saveStatus === 'error' && <div><p className="font-semibold">We couldn’t save {displayName}’s plan.</p><p className="mt-1 text-sm text-[var(--cream)]/80">Your plan is still here. Please try again.</p></div>}</div></section>}
  </div>;
};

...
interface HousingStep3Props {
  selectedGoal: HousingGoal;
  onSelectGoal: (goal: HousingGoal) => void;
  onContinue: () => void;
  onBack: () => void;
}

export const HousingStep3: React.FC<HousingStep3Props> = ({
  selectedGoal,
  onSelectGoal,
  onContinue,
  onBack,
}) => {
  const options: HousingGoal[] = [
    "Stay where I am",
    "Move",
    "Either could work",
  ];
    ...
    <div className="py-8 sm:py-14 md:py-20 px-4 sm:px-6 md:px-8 max-w-3xl mx-auto w-full">
      ...
      <div className="mb-8 sm:mb-10">
        <h1 className="font-serif text-3xl sm:text-4xl md:text-5xl text-[#2E5440] font-normal leading-tight tracking-tight mb-3 sm:mb-4 text-balance">
          Are you trying to stay where you are or move?
        </h1>
        <p className="font-sans text-base sm:text-lg text-[#2D2D2D]/80 leading-relaxed text-pretty">
          We’ll use this to focus the next steps on what may be most useful.
        </p>
      </div>
        ...
        <div className="pt-2 flex flex-col sm:flex-row items-center gap-4">
          ...
          {!isContinueEnabled && (
            <p className="text-xs text-[#2D2D2D]/60 italic font-sans">
              Please select an option to see your options.
            </p>
          )}
        </div>
      </form>

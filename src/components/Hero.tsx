...
export const Hero: React.FC<HeroProps> = ({ onStart }) => (
  <section className="overflow-hidden border-b border-[var(--border-warm)]/70 bg-[var(--cream)]"><div className="mx-auto grid max-w-[1380px] lg:grid-cols-[51%_49%]">
    ...
    <Reveal direction="right" delay={0.1} className="relative min-h-[590px] lg:min-h-[600px]">
      <img src="/images/pets-resting-hero.jpg" alt="A golden retriever and tabby cat resting together" width="1672" height="941" decoding="async" className="landing-hero-photo absolute inset-0 h-full w-full object-cover object-center" />
      ...
      <div aria-label="Example of Keep Them Home path results" className="absolute bottom-6 left-1/2 w-[min(430px,calc(100%-3rem))] -translate-x-1/2 rounded-[22px] bg-white/95 p-5 shadow-[0_22px_55px_rgba(45,45,45,.22)] backdrop-blur-sm lg:bottom-6 lg:left-4 lg:translate-x-0 xl:left-9">
        ...
        <ol className="mt-3 space-y-2">{paths.map(([number, title, copy, status, tone], index) => <Reveal key={number} delay={0.34 + index * 0.06}><li className="grid grid-cols-[30px_1fr_auto] items-center gap-3 rounded-xl bg-[var(--surface-soft)]/75 px-3 py-2"><span className={`flex h-7 w-7 items-center justify-center rounded-full text-sm font-bold ${tone === 'blocked' ? 'bg-[var(--status-blocked-bg)] text-[var(--status-blocked-text)]' : 'bg-[#dcebd8] text-[var(--forest)]'}`}>{number}</span><span><strong className="block text-xs text-[var(--charcoal)] sm:text-[13px]">{title}</strong><span className="block text-[10px] leading-tight text-[var(--text-muted)] sm:text-[11px]">{copy}</span></span><StatusBadge status={status} className="px-2 py-1 text-[9px]" /></li></Reveal>)}</ol>

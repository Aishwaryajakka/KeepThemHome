...
const AccountMenu = () => <UserButton><UserButton.MenuItems><UserButton.Link href="/my-pets" label="My Pets" labelIcon={<PawPrint className="h-4 w-4" />} /></UserButton.MenuItems></UserButton>;
...
export const Header: React.FC<HeaderProps> = ({
  ...
  if (variant === 'product') return <header className="sticky top-0 z-30 w-full border-b border-[var(--border-warm)]/70 bg-[var(--cream)]/95 backdrop-blur-md">
    <div className="mx-auto flex min-h-[68px] max-w-[1360px] items-center justify-between gap-3 px-4 sm:px-6 lg:px-8">
      <a href="/" onClick={(event) => { if (onCtaClick) { event.preventDefault(); onCtaClick(); } }} className="brand-focus rounded-md" aria-label="Keep Them Home Homepage"><BrandLockup compact /></a>

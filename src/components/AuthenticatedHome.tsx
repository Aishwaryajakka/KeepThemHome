...
export default function AuthenticatedHome({ active, onStart, onContinue, onViewAll }: AuthenticatedHomeProps) {
  ...
  return <section className="border-b border-[var(--border-warm)] bg-white/55 py-10 sm:py-12" aria-labelledby="welcome-back-heading"><div className="mx-auto max-w-[1380px] px-5 sm:px-8 lg:px-12">
    ...
    {status === 'ready' && cases.length > 0 && <div className="mt-6 grid gap-4 md:grid-cols-2 lg:grid-cols-3">{cases.map((item) => { const path = paths[item.case.id]; return <article key={item.case.id} className="rounded-2xl border border-[var(--sage)]/35 bg-[var(--cream)] p-5"><div className="flex items-start justify-between gap-3"><div><h3 className="font-serif text-2xl text-[var(--forest)]">{item.pet.name}</h3><p className="mt-1 flex items-center gap-1.5 text-sm capitalize text-[var(--text-muted)]"><PawPrint className="h-3.5 w-3.5" aria-hidden="true" />{item.pet.type}</p></div>{path && <StatusBadge status={path.status} />}</div><p className="mt-4 font-medium text-[var(--forest)]">{path?.title ?? `${factorLabel(item.case.primaryBarrier ?? '')} plan`}</p><Button type="button" variant="outline" onClick={() => onContinue(item.case.id)} className="mt-5 w-full border-[var(--forest)] text-[var(--forest)]">Continue {item.pet.name}’s plan <ArrowRight className="ml-2 h-4 w-4" aria-hidden="true" /></Button></article>; })}</div>}
  </div></section>;
}
